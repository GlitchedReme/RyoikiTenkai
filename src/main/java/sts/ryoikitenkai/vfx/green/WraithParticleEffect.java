...
public class WraithParticleEffect extends AbstractGameEffect {
    private float x;
    private float y;

    private static TextureAtlas.AtlasRegion IMG = ImageMaster.ROOM_SHINE_2;

    public WraithParticleEffect(float x, float y) {
        this.duration = MathUtils.random(0.5F, 1.0F);
        this.startingDuration = this.duration;
        this.x = x - (IMG.packedWidth / 2);
        this.y = y - (IMG.packedHeight / 2);
        this.scale = Settings.scale * MathUtils.random(0.3F, 0.4F);
        this.rotation = 0.0F;
        this.color = new Color(MathUtils.random(0.8F, 1.0F), MathUtils.random(0.1F, 0.2F), MathUtils.random(0.1F, 0.2F), 0.01F);
    }
