...
public class FormParticleEffect extends AbstractGameEffect {
    private TextureAtlas.AtlasRegion img;
    private float x;
    private float y;
    private float delayTimer;

    public FormParticleEffect(float playerX, Color color) {
        ...
        if (this.delayTimer > 0.0F) {
            ...
            if (this.duration < 0.0F) {
                ...
                if (this.duration > this.startingDuration / 2.0F) {
                    this.color.a = Interpolation.pow3In.apply(0.6F, 0.0F,
    ...
    public void render(SpriteBatch sb) {
        if (!(this.delayTimer > 0.0F)) {
            ...
            sb.draw(this.img, AbstractDungeon.player.hb.cX + this.x, this.y, (float) this.img.packedWidth / 2.0F,
                    (float) this.img.packedHeight / 2.0F, (float) this.img.packedWidth, (float) this.img.packedHeight,
                    this.scale * MathUtils.random(2.9F, 3.1F), this.scale * MathUtils.random(0.95F, 1.05F), this.rotation);
