...
public class OrbEffect extends AbstractGameEffect {
    ...
    public OrbEffect(float x, float y, int num, Color color) {
        this.img = ImageMaster.GLOW_SPARK_2;
        this.pos = new Vector2(x, y);
        this.target = new Vector2(Settings.WIDTH / 2.0F + 1400.0F * (float) Math.cos(num * 10.0F) - DST_THRESHOLD / 3.0F - 100.0F * Settings.scale,
                Settings.HEIGHT / 2.0F + 1400.0F * (float) Math.sin(num * 10.0F) + MathUtils.random(-50.0F, 50.0F) * Settings.scale);
    ...
    public void render(SpriteBatch sb) {
        if (!this.isDone) {
            ...
            float scale = Settings.scale * 2F;

            for (int i = this.points.length - 1; i > 0; --i) {
                if (this.points[i] != null) {
                    sb.draw(this.img, this.points[i].x - (float) (this.img.packedWidth / 2), this.points[i].y - (float) (this.img.packedHeight / 2),
                            (float) this.img.packedWidth / 2.0F, (float) this.img.packedHeight / 2.0F, (float) this.img.packedWidth,
